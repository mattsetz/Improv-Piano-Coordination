...
to_replace = {'c6a4': 'c6a3','b4d6': 'b3d6','c7a6': 'c7a4','b6d7': 'b4d7','o2p2': 'o2n2'}
invalid = ['c6a4','b4d6','c7a6','b6d7','o2p2']
valid = ['c6a3', 'b3d6', 'c7a4', 'b4d7', 'o2n2']
def is_invalid(f):
    for i in invalid:
        if (i in f): return True
    return False
...
onset_files = [f for f in glob('../Pipeline/onsets/*.csv') if is_invalid(f)]
for f in onset_files:
    new_name = f
    for k in to_replace:
        new_name = new_name.replace(k,to_replace[k])
    os.rename(f,new_name)

# iterate through every previously invalid file, add onsets to master_onsets
