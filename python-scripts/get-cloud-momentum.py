...
OUT_DIR = "../Pipeline/tonal/cloud-momentum/individual/"
...
def get_cloud_momentum_window(pc_df, window, topology):
	pc_df = pc_df.loc[pc_df['window']==window]

	for i 
	return

def get_cloud_momentum(filein, topology):
	# read in interval class (ic) dataframe
	pc_df = pd.read_csv(filein)

	# compute cloud momentum at each window
	cloud_momentum_df = pd.DataFrame()
	windows = np.unique(pc_df.window)
	for w in windows:
		cloud_momentum_df = pd.concat((cloud_momentum_df, 
			get_cloud_momentum_window(pc_df, w, topology)))

	# write dataframe to csv
	fileout = filein.split("/")[-1].replace('-pc-window.csv','-cloud-momentum.csv')
	cloud_momentum_df.to_csv(OUT_DIR+fileout, index=False)
	return
...
for f in infiles:
	print(f)
	get_cloud_momentum(f, diss_dict_chew_2_15)