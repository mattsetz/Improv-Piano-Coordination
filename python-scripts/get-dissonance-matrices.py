...
def get_dissonance_matrix(filepath1, filepath2, dyad_name, player1_name,WINDOW_SIZE):
	...
	diss_m.to_csv(OUT_DIR+dyad_name+'.'+player1_name+'-'+str(WINDOW_SIZE)+'-sec-diss-matrix.csv', index=False)
	return
...
for window in windows:
	...
	for f in infiles:
		dyad_name = f.split('.')[-3]
		if dyad_name in [s.split('/')[-1].split('-')[0] for s in glob(OUT_DIR+"*")]:
			continue
		...
		get_dissonance_matrix(f,file2,dyad_name,this_person,window)
		get_dissonance_matrix(file2,f,dyad_name,other_person,window)