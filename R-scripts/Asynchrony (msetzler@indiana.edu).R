...
# analyze asyncs in one-way trials
asyncs$person <- str_remove_all(asyncs$person_trial,'[0-9]')
asyncs.oneway <- asyncs %>% filter(condition=="one-way")
head(asyncs.oneway)
library(lme4)
library(lmerTest)
asyncs.model <- lmer(asyncs ~ 1 + (1|person), data=asyncs.oneway)
summary(asyncs.model)

library(brms)
asyncs.model.brms <- brm(asyncs ~ 1 + (1|person), data=asyncs.oneway)
