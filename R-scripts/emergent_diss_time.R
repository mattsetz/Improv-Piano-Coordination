...
diss.avg <- diss %>% group_by(pair,window,condition,n_time20) %>% summarise(diss.comb=mean(diss.comb,na.rm = TRUE),diss.emerge=mean(diss.emerge,na.rm=TRUE)) %>% ungroup()
ggplot(filter(diss.avg,window==5), aes(x=5*n_time20,y=1.8-diss.emerge,fill=condition)) + stat_summary(geom="ribbon",alpha=.4) + stat_summary(geom="point",aes(color=condition)) + stat_summary(geom="line",aes(color=condition)) + theme_bw() + theme(legend.position = c(.8,.2)) + ylab("Emergent Consonance") + xlab("time (%)")
diss.avg <- diss %>% group_by(pair,window,condition,n_time50) %>% summarise(diss.comb=mean(diss.comb,na.rm = TRUE),diss.emerge=mean(diss.emerge,na.rm=TRUE)) %>% ungroup()
ggplot(filter(diss.avg,window==5), aes(x=2*n_time50,y=1.8-diss.emerge,fill=condition)) + stat_summary(geom="ribbon",alpha=.4) + stat_summary(geom="point",aes(color=condition)) + stat_summary(geom="line",aes(color=condition)) + theme_bw() + theme(legend.position = c(.8,.2)) + ylab("Emergent Consonance") + xlab("time (%)")
ggplot(filter(diss,window==5), aes(x=n_time100,y=diss.emerge,color=condition)) + stat_summary() + xlab('Normalized Time (%)') + ylab("Emergent Dissonance")

diss.avg.w <- diss %>% filter(window==5) %>% group_by(pair,condition,n_time20) %>% summarise(diss.emerge=mean(diss.emerge,na.rm = TRUE)) %>% ungroup()
ggplot(diss.avg.w,aes(x=n_time20,y=diss.emerge,color=condition)) + stat_summary()
...
fit.quad.model <- function(ntime, window_size) {
  diss.w.avg <- diss %>% filter(window==window_size) %>% rename(n_time=ntime) %>% group_by(pair,n_time,condition) %>% summarise(diss.emerge=mean(diss.emerge,na.rm = TRUE)) %>% ungroup() %>% drop_na()
...
samples <- get(load("Analysis/jags/quad_fits_max_ess/MultiConditionQuadEmergentDissonanceCodaSamples_window5_n_time50.rds"))
effectiveSize(samples)
# gelman.diag
...
fits$ntime_factor <- fits$ntime %>% factor(labels=c("10 time bins","20 time bins","50 time bins","100 time bins"))
fits$window_factor <- fits$window_size %>% factor(labels=c("2 sec window","5 sec window","10 sec window","20 sec window"))
fits <- fits %>% arrange(ntime_factor,window_factor)
ggplot(fits, aes(x=-beta1_cond1_cond2)) + geom_density() + facet_grid(rows=vars(ntime_factor),cols=vars(window_factor)) + theme_bw() + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) + xlab("Emergent Consonance ~ Time. coupled minus oneway") + ylab("Normalized Frequency")
ggplot(fits, aes(x=-beta2_cond1_cond2)) + geom_density() + facet_grid(rows=vars(ntime_factor),cols=vars(window_factor),scales = "free") + theme_bw() + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) + xlab("Emergent Consonance ~ Time^2. coupled minus oneway") + ylab("Normalized Frequency")
